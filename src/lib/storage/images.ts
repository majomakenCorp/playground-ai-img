...
import { r2Configured, r2Put, r2GetBytes, r2Head } from "./r2";
...
function filenameFor(id: string, ext: string, variant?: string): string {
  return variant ? `${id}.${variant}.${ext}` : `${id}.${ext}`;
}
...
export async function writeImage(args: {
  ...
  const filename = filenameFor(args.id, args.ext, args.variant);
  const mime = EXT_TO_MIME[args.ext];

  if (r2Configured()) {
    await r2Put(filename, args.bytes, mime);
    return filename;
  }
...
export async function resolveImage(
  ...
  if (r2Configured()) {
    for (const ext of EXTS) {
      const filename = filenameFor(id, ext, variant);
      if (await r2Head(filename)) {
        return { filename, ext, mime: EXT_TO_MIME[ext] };
      }
    }
    return null;
  }
